...
(define (execute-code addr . interpreter)
  (push-int! rstack (inexact->exact (ceiling (/ pc 4)))) ; pc will be the address of the next instruction to execute
  (set-pc! addr))

(define (execute entry . interpreter)
  (execute-code (entry-code entry) interpreter))
...
(define (add-compiled-code! elmt . compiler)
...
(define (fill-rest-with-nops . compiler)
  (unless (= (remainder i-register 4) 0)
	  (add-compiled-code! "." compiler)
	  (fill-rest-with-nops compiler)))

(define (fill-rest-with-false . compiler)
  (unless (= (remainder i-register 4) 0)
	  (add-compiled-code! #f compiler)
	  (fill-rest-with-false compiler)))
...
(define (compile code-port)
  ...
  (add-compiler-directives! compiler)
    ...
    (let [(to-compile (forth_read))]
      (unless (eof-object? to-compile)
	      (unless (eq? to-compile #\newline)
			...
			(if directive
			    ((rvector-ref codespace (entry-code directive)) compiler)
			      ...
			      (cond [(not entry)
				    ...
				    [(member to-compile address-required-on-cstack)
				     (when (= (remainder i-register 4) 3)
					   (fill-rest-with-nops compiler))
				     (add-compiled-code! to-compile compiler)
				     (compile-address! (pop-int! cstack #f) compiler)
				     (fill-rest-with-nops compiler)]
					...
					; This is already taken care of by add-compiled-code!
				       (if (equal? nxt ";")
					   (add-compiled-code! "jump" compiler)
					...
					; Compile the address.  Automatically compiles #f into the rest of the word.
				       (compile-address! (entry-code entry) compiler))])))))
	      (compile-loop))))
  (set! used-cores '())
